...
const validate = async (query: ImageQuery): Promise<null | string> => {
  ...
  if (!(await File.isImageAvailable(query.filename))) {
    ...
    return `Invalid filename pass a valid filename in the 'filename' query segment. Filename you may use are: ${availableImageNames}.`;
  }
  ...
  if (Number.isNaN(width) || width < 1) {
    return "width only takes positive integers.";
  }

  // Check for valid height value
  const height: number = parseInt(query.height || '');
  if (Number.isNaN(height) || height < 1) {
    return "height only takes positive integers";
  }
